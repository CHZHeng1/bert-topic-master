...
from model.down_stream_tasks.bert_classification import BertWithDocumentWeiboUser
...
def train(config):
    ...
    # 2.数据处理
    model = BertWithDocumentWeiboUser(config, config.pretrained_model_dir)
    model_save_path = os.path.join(config.model_save_dir, 'bert_document_weibo_user.pt')
    ...
    train_iter, test_iter, val_iter = \
        data_loader.load_data_with_document_and_lr(train_file_path=config.train_file_path,
    ...
    for epoch in range(config.epochs):
        ...
        for idx, (sample, label, topic_vector, feature_variable) in enumerate(train_iter):
...
def inference(config):
    ...
    #     save_csv('labels_document', label)
...
def evaluate(data_iter, model, device, PAD_IDX):
    ...
    with torch.no_grad():
        y_true, y_pred = [], []
        test_losses = 0
        for sample, label, topic_vector, feature_variable in tqdm(data_iter, desc='Evaluating'):
            sample = sample.to(device)  # [src_len, batch_size]
