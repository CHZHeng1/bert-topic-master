...
from topic_model_sklearn.topic_loader import load_word_topics
...
def train(config, feature_name=None):
    ...
    for epoch in range(config.epochs):
        ...
        for idx, (sample_bert, label, sample_topic, influence_timely, reliability, influence) in enumerate(train_iter):
            ...
            influence_timely = influence_timely.to(config.device)
            reliability = reliability.to(config.device)
            influence = influence.to(config.device)
            ...
            loss, logits = model(input_ids=sample_bert,
                                 ...
                                 influence_timely=influence_timely,
        ...
        acc, p, r, f, _, _ = \
            evaluate(val_iter, model, config.device, data_loader.PAD_IDX, topic_matrix_tensor, feature_name)
...
def inference(config, feature_name=None):
    ...
    acc, p, r, f, y_true, y_pred = \
        evaluate(test_iter, model, config.device, data_loader.PAD_IDX, topic_matrix_tensor, feature_name=feature_name)
...
def evaluate(data_iter, model, device, PAD_IDX, topic_matrix_tensor, feature_name=None):
    model.eval()
    with torch.no_grad():
        y_true, y_pred = [], []
        test_losses = 0
        for sample_bert, label, sample_topic, influence_timely, reliability, influence in tqdm(data_iter, desc='Evaluating'):
            sample_bert = sample_bert.to(device)
            label = label.to(device)
            sample_topic = sample_topic.to(device)
            influence_timely = influence_timely.to(device)
...
if __name__ == '__main__':
    ...
    train(model_config, feature_name='BERT-W-I(t)-R')  # BERT-W-I-R or BERT-W-I(t)-R
    inference(model_config, feature_name='BERT-W-I(t)-R')
