...
from model.bert_model.bert_embedding import BertEmbeddings
from model.bert_model.my_transformer import MyMultiheadAttention
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        """
        all_encoder_layers = []
        layer_output = hidden_states  # 当前层的输入为上一层的输出
...
class BertModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.bert_pooler = BertPooler(config)  # bert
        self.config = config
    ...
    def from_pretrained(cls, config, pretrained_model_dir=None):
        ...
        state_dict = deepcopy(model.state_dict())  # 深拷贝
        loaded_paras_names = list(loaded_paras.keys())[:-8]  # 取bert-base-chinese前199个参数 （共207个参数）
        model_paras_names = list(state_dict.keys())[1:]  # bert-base后199个参数 （共200个参数）
        # print(len(loaded_paras_names))  # 199
        # print(len(model_paras_names))  # 151
        if 'use_torch_multi_head' in config.__dict__ and config.use_torch_multi_head:
            torch_paras = format_paras_for_torch(loaded_paras_names, loaded_paras)
            # print(len(torch_paras))  # 151
...
#     print("\n  =======  BertMolde 参数: ========")
#     print(len(BertModel.state_dict()))
#     for param_tensor in BertModel.state_dict():
#         print(param_tensor, "\t", BertModel.state_dict()[param_tensor].size())
