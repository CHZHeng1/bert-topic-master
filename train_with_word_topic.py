...
from model.down_stream_tasks.bert_classification import BertWithWordTopics
...
def train(config):
    ...
    # 2.数据处理
    model = BertWithWordTopics(config, config.pretrained_model_dir)
    model_save_path = os.path.join(config.model_save_dir, 'model_word_topic.pt')
    ...
    train_iter, test_iter, val_iter = \
        data_loader.load_data_with_word_topics(config, train_file_path=config.train_file_path,
    ...
    for epoch in range(config.epochs):
        ...
        for idx, (sample_bert, label, sample_topic, sample_topic_length) in enumerate(train_iter):
            ...
            sample_topic_length = sample_topic_length.to(config.device)
            ...
            loss, logits = model(input_ids=sample_bert,
                                 ...
                                 word_topic_length=sample_topic_length,
...
def inference(config):
    ...
    data_loader = LoadSingleSentenceClassificationDataset(vocab_path=config.vocab_path,
                                                          ...
                                                          is_sample_shuffle=config.is_sample_shuffle)
    train_iter, test_iter, val_iter = \
        data_loader.load_data_with_word_topics(config,
...
def evaluate(data_iter, model, device, PAD_IDX, topic_matrix_tensor):
    ...
    with torch.no_grad():
        y_true, y_pred = [], []
        test_losses = 0
        for sample_bert, label, sample_topic, sample_topic_length in tqdm(data_iter, desc='Evaluating'):
            sample_bert = sample_bert.to(device)
            label = label.to(device)
            sample_topic = sample_topic.to(device)
            sample_topic_length = sample_topic_length.to(device)
